...
function updateToStart() {
    for (let i = 0; i < script.api.tweenObjects.length; i++) {
        const tweenObject = script.api.tweenObjects[i];
...
function startTween() {
    ...
    if (tween) {
        if (script.api.tween.length > 0) {
            ...
            // Start the tweens
            for (let i = 0; i < script.api.tween.length; i++) {
...
function setupAlphaComponentTweens(componentType, sceneObject) {
    const visualComponents = sceneObject.getComponents(componentType);

    for (let i = 0; i < visualComponents.length; i++) {
        const visualComponent = visualComponents[i];
        let startValue = null;
        let endValue = null;
        let tween = null;
    ...
    if (script.recursive) {
        for (let j = 0; j < sceneObject.getChildrenCount(); j++) {
...
function setupTweenBackwards() {
    const tempTweenObjectsArray = [];

    const tempTweenArray = [];

    const easingType = global.tweenManager.getSwitchedEasingType(script.easingType);

    for (let i = 0; i < script.api.tween.length; i++) {
        let newTweenObject = null;

        const tween = script.api.tweenObjects[i];

        const tweenStart = (script.loopType == 3) ? tween.startValue : tween.endValue;

        const tweenEnd = (script.loopType == 3) ? tween.endValue : tween.startValue;

        const tweenEasingType = global.tweenManager.getTweenEasingType(script.easingFunction, easingType);

        const newTween = new global.TWEEN.Tween(tweenStart)
...
function updateText(visualComponent, value) {
    const fillColor = visualComponent.textFill.color;
    ...
    // Outline Color
    const outlineColor = visualComponent.outlineSettings.fill.color;
    ...
    // Drop Shadow Color
    const dropShadowColor = visualComponent.dropshadowSettings.fill.color;
    ...
    // Background Color
    const backgroundColor = visualComponent.backgroundSettings.fill.color;
...
function updateVisual(visualComponent, value) {
    const currColor = visualComponent.getMaterial(0).getPass(0).baseColor;
