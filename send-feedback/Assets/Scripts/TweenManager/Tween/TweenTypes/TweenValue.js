...
function setupTween() {
    ...
    if (!script.api.manualStart) {
        const startRaw = getRawByName('start');
        script.api.start = getValueFromRaw(startRaw);

    }

    if (!script.api.manualEnd) {
        const endRaw = getRawByName('end');
        script.api.end = getValueFromRaw(endRaw);
    }

    const startValue = script.api.start;
    const endValue = script.api.end;
...
function setupTweenBackwards() {
    const startValue = (script.loopType == 3) ? script.api.start : script.api.end;

    const endValue = (script.loopType == 3) ? script.api.end : script.api.start;
...
function updateValue(value) {
    if (script.propertiesCount <= 1) {
        ...
        for (let i = 0; i < script.propertiesCount; i++) {
...
function getRawByName(valueName) {
    const value = startEndValues[valueName][script.dataType];
