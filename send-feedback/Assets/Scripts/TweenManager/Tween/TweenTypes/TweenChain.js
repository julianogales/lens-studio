...
function updateToStart() {
    ...
    for (let i = script.tweenNames.length - 1; i >= 0; i--) {
        const scriptComponent = (script.recursive) ? global.tweenManager.findTweenRecursive(script.api.sceneObjectReference, script.tweenNames[i]) : global.tweenManager.findTween(script.api.sceneObjectReference, script.tweenNames[i]);
...
function updateToEnd() {
    for (let i = 0; i < script.tweenNames.length; i++) {
        const scriptComponent = (script.recursive) ? global.tweenManager.findTweenRecursive(script.api.sceneObjectReference, script.tweenNames[i]) : global.tweenManager.findTween(script.api.sceneObjectReference, script.tweenNames[i]);
...
function startTween() {
    ...
    if (script.api.firstTween) {
        if (script.playAll) {
            for (let i = 0; i < script.api.allTweens.length; i++) {
                script.api.allTweens[i].start();
            }
        } else {
            if (Array.isArray(script.api.firstTween)) {
                for (let j = 0; j < script.api.firstTween.length; j++) {
...
function setupTween() {
    const result = chainTweensInOrder(script.playAll, script);
...
function chainTweens(order, playAll, originalScript) {
    ...
    let longestTween = null;
    let firstTweenFound = false;
    const orderCondition = (order == 'forwards');
    const tweenNames = script.tweenNames.slice();
    if (!orderCondition) {
        tweenNames.reverse();
    }
    // Tweens are created in order specified by the parameter
    for (let i = 0; i < tweenNames.length; i++) {
        const scriptComponent = (script.recursive) ? global.tweenManager.findTweenRecursive(script.api.sceneObjectReference, tweenNames[i]) : global.tweenManager.findTween(script.api.sceneObjectReference, tweenNames[i]);
...
function chainTweensPingPongOnce(originalScript) {
    ...
    const forwardsResult = chainTweensInOrder(false, originalScript);
    updateToStart();
    const backwardsResult = chainTweensBackwards(false, originalScript);
...
function setupPingPong(playAll, originalScript) {
    const repeatPingPong = function() {
        script.api.backwards = !script.api.backwards;
        if (script.api.backwards) {
            const result = chainTweensBackwards(playAll, originalScript);
            script.api.firstTween = result.firstTween;
            script.api.lastTween = result.lastTween;
            script.api.allTweens = result.allTweens;
            script.api.longestTween = result.longestTween;
...
function setupPingPongOnce(playAll, originalScript) {
    const repeatPingPongOnce = function() {
        script.api.backwards = !script.api.backwards;
        if (script.api.backwards) {
            const result = chainTweensBackwards(playAll, originalScript);
            script.api.firstTween = result.firstTween;
