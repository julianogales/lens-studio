...
const propertyTypes = ['Position', 'Scale', 'Rotation', 'Anchors', 'Offsets'];
const PropertyType = {};
propertyTypes.forEach(function(d, i) {
    PropertyType[d] = i;
});

const RectPropTypes = ['Bounds', 'Size', 'Center'];

const vecProperties = ['x', 'y', 'z', 'w'];

const movementProperties = ['start', 'to', 'from', 'offset'];

const propertyTypeName = Object.keys(PropertyType)[script.type];
...
function setupTween() {
    ...
    let tween = null;
    const screenTransform = script.api.sceneObjectReference.getComponent('Component.ScreenTransform');
    let screenTransformParams = null;
...
function createObjectFromVec(vec) {
    const value = {};
    for (let i = 0; i < vecProperties.length; i++) {
