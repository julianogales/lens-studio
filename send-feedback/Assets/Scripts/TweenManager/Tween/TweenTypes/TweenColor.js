...
function getVisualComponentEndColor(visualComponent, startValue) {
    if (visualComponent.getTypeName() == 'Component.Text') {
        const textSetting = getTextSetting(visualComponent);
        return {
...
function setupTweenBackwards() {
    ...
    for (let i = 0; i < script.api.tween.length; i++) {
        ...
        const newTween = new global.TWEEN.Tween((script.loopType == 3) ? tween.startValue : tween.endValue)
...
function updateText(visualComponent, value) {
    ...
    if (script.ignoreAlpha) {
        const currColor = textSetting.color;
