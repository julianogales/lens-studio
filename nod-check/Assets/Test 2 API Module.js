 ...
 * // Instead, you should modify the "Test 2 API" script and access the functions through the imported class wrapper.
*/

/*
 * @param {RemoteApiResponse} response A raw API response from an Test 2 API
...
ApiModule.prototype.get_all = function(cb) {
    var req = global.RemoteApiRequest.create();
    req.endpoint = "get_all";
...
ApiModule.prototype.post_feedback = function(cb) {
    var req = global.RemoteApiRequest.create();
    req.endpoint = "post_feedback";
    this.remoteServiceModule.performApiRequest(req, function(response) {
        if(cb) {
            handleAPIResponse(response, cb);
        }
